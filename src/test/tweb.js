...
(async () => {
  ...
  ok('en la compu sale la ventanita con el link', await A.evaluate(c => { const e = document.getElementById('invite'); return !!e && e.querySelector('.inv-url').value.endsWith('?sala=' + c); }, code));
  await key(A, 'Escape', 200);
